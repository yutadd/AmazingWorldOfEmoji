...
function App() {
  ...
  let init: ContextType = [right, setRight, showL, setShowL, name, setName];
  ...
  function loginDialog() {
    return (
      ...
      >
        ...
        ></TextField>
        <br />
        <Button variant="contained" onClick={() => login(inputName, inputPass)}>
          Login
        </Button>
  ...
  function login(name: string, pass: string) {
    fetch("/api/user/post/login?id=" + name + "&pass=" + pass, {
      method: "POST",
    }).then((e) =>
      e.text().then((t) => {
        if (t == "accepted") {
